    ...
    <section class="mt-[60px] px-7 pt-8 pb-12 font-sans">
        ...
        <div class="flex flex-wrap items-end justify-between gap-4 mb-7">
            <div>
                ...
                <p class="text-[0.82rem] text-text-muted mt-1">Kelola kelompok menu dan kategori produk warung</p>
            </div>
            <a href="{{ route('tambahKategori') }}"
                class="inline-flex items-center gap-2 px-5 py-2.5 bg-secondary text-white rounded-[10px] text-[0.85rem] font-semibold shadow-[0_3px_12px_rgba(92,122,78,0.2)] transition-all duration-200 hover:bg-secondary-dark hover:-translate-y-0.5 whitespace-nowrap">
        ...
        <div class="bg-base border border-border rounded-2xl overflow-hidden shadow-[0_2px_16px_rgba(44,36,22,0.05)]">
            ...
            <div class="overflow-x-auto">
                <table class="w-full border-collapse text-[0.875rem]" id="produk-table">
                    <thead>
                        <tr
                            class="bg-surface border-b-2 border-border text-text-muted uppercase text-[0.68rem] font-bold tracking-widest">
                    ...
                    <tbody class="divide-y divide-border">
                        @foreach ($dataKategori as $item)
                            <tr class="transition-colors hover:bg-surface/50">
                                ...
                                <td class="px-6 py-4">
                                    <span
                                        class="text-[0.75rem] font-semibold text-text-muted bg-surface-alt border border-border rounded px-2 py-0.5">
                                        #{{ $item->id_kategori }}
                                    </span>
                                </td>
                                ...
                                <td class="px-6 py-4 text-text-muted text-xs">
                                    ...
                                    <span class="block opacity-60 text-[10px]">{{ $item->created_at->format('H:i') }}
                                        WIB</span>
                                </td>
                                ...
                                <td class="px-6 py-4 text-text-muted text-xs">
                                    ...
                                    <span class="block opacity-60 text-[10px]">{{ $item->updated_at->format('H:i') }}
                                        WIB</span>
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        <a href="{{ route('kategoriUpdate', $item->id_kategori) }}"
                                            class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[0.78rem] font-medium border border-border bg-base text-text-muted transition-all hover:text-secondary hover:border-secondary hover:bg-secondary/5 whitespace-nowrap">
                                        ...
                                        <form onsubmit="return confirm('Apakah Anda yakin ingin menghapus Kategori ini?');"
                                            ...
                                            @method('DELETE')
                                            <button type="submit"
                                                class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[0.78rem] font-medium border border-[#FAD8D5] bg-[#FFF0EE] text-primary-dark transition-all hover:bg-primary-dark hover:text-white whitespace-nowrap group">
                                                <svg width="13" height="13" viewBox="0 0 24 24" fill="none"
                                                    stroke="currentColor" stroke-width="2.5" stroke-linecap="round">
            ...
            {{-- Footer --}}
            <div
                class="px-6 py-4 bg-surface border-t border-border flex justify-between items-center text-[0.72rem] text-text-light italic font-medium">
